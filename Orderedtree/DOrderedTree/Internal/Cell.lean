...
theorem ext [Ord α] {k : α} {c c' : Cell α β k} : c.inner = c'.inner → c = c' := by
  cases c; cases c'; simp

end Cell

def List.findCell [Ord α] (l : List ((a : α) × β a)) (k : α) : Cell α β k where
  inner := l.find? (compare k ·.1 == .eq)
  property p hp := by simpa using (List.find?_eq_some_iff_append.1 hp).1

theorem List.findCell_inner [Ord α] (l : List ((a : α) × β a)) (k : α) :
    (findCell l k).inner = l.find? (compare k ·.1 == .eq) := rfl

@[simp] theorem List.findCell_nil [Ord α] (k : α) : (findCell [] k : Cell α β k) = .empty := rfl
